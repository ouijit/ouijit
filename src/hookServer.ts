...
export const CODEX_WRAPPER = [
  ...
  'CLEAN_PATH="${CLEAN_PATH%:}"',
  '',
  '# Resolve the real codex. The PATH strip above is pure string match, so',
  '# a different spelling of the wrapper dir (trailing slash, symlink target,',
  '# stray duplicate) can leave the wrapper reachable and cause us to exec',
  '# ourselves. Each recursion appends ~6.5 KB of -c overrides to argv until',
  '# execve hits ARG_MAX and fails with E2BIG. Compare by inode (`-ef`) so',
  '# every spelling resolves to the same identity check.',
  'REAL_CODEX="$(PATH="$CLEAN_PATH" command -v codex 2>/dev/null)"',
  'if [ -n "$REAL_CODEX" ] && [ "$REAL_CODEX" -ef "$WRAPPER_SELF" ]; then',
  '  REAL_CODEX=""',
  'fi',
  ...
  '    [ -z "$_dir" ] && continue',
  '    if [ -x "$_dir/codex" ] && [ ! "$_dir/codex" -ef "$WRAPPER_SELF" ]; then',
  '      REAL_CODEX="$_dir/codex"',
  '      break',
  '    fi',
  '  done',
  ...
  'fi',
  'if [ -z "$REAL_CODEX" ]; then',
  '  echo "ouijit: codex not found on PATH (or only the Ouijit wrapper was found)" >&2',
  '  exit 1',
  'fi',
  '',
