...
export const ZSH_INTEGRATION = [
  ...
  '',
  '# Emit OSC 133;D;<exit_code> for the prior command (if any) and OSC 133;A',
  '# for the new prompt about to be drawn. ;D is skipped on the very first',
  '# prompt since no user command has run yet; ;A always fires so the renderer',
  '# can mark idle state from the first prompt onward. MUST be the first',
  '# precmd so $? still reflects the user command, not a downstream precmd.',
  ...
  '',
  '# Emit OSC 133;C just before each command runs so the renderer can',
  "# distinguish 'at prompt' (idle) from 'command running'. preexec is zsh's",
  '# standard pre-command hook.',
  '_ouijit_emit_command_start() {',
  '  printf "\\033]133;C\\007"',
  '}',
  'preexec_functions+=(_ouijit_emit_command_start)',
  '',
...
export const BASH_INTEGRATION = [
  ...
  '# Emit OSC 133;D;<exit_code> for the prior command (if any) and OSC 133;A',
  '# for the new prompt about to be drawn. ;D is skipped on the very first',
  '# prompt; ;A always fires so the renderer can mark idle state from the',
  '# first prompt onward. Prepended to PROMPT_COMMAND so $? still reflects',
  '# the user command rather than a previously-installed hook.',
  '_ouijit_emit_exit_code() {',
  '  local code=$?',
  '  if [ -n "$_OUIJIT_HAS_RUN" ]; then',
  '    printf "\\033]133;D;%d\\007" "$code"',
  ...
  '',
  '# Emit OSC 133;C just before each command runs so the renderer can',
  "# distinguish 'at prompt' (idle) from 'command running'. PS0 is bash's",
  '# pre-command prompt-string (bash 4.4+); older bash silently ignores it',
  '# and degrades to ;D-only tracking. \\e and \\a are standard bash prompt',
  '# escapes for ESC and BEL.',
  'PS0=\'\\[\\e]133;C\\a\\]\'"$PS0"',
  '',
  '# When we exec into this shell from a one-off command (a hook script), the',
  '# subshell exit code is passed across the exec via OUIJIT_INITIAL_EXIT. Emit',
