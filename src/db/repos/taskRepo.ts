...
export class TaskRepo {
  ...
  getAllForProject(projectPath: string): TaskRow[] {
    return this.db.prepare(
      'SELECT * FROM tasks WHERE project_path = ? ORDER BY sort_order, id'
  ...
  create(
    ...
    return this.db.transaction(() => {
      ...
      const maxOrder = this.db.prepare(
        'SELECT MAX(sort_order) as max_order FROM tasks WHERE project_path = ? AND status = ?'
      ...
      this.db.prepare(
        'UPDATE project_counters SET next_task_number = MAX(next_task_number, ? + 1) WHERE project_path = ?'
  ...
  updateStatus(projectPath: string, taskNumber: number, status: TaskStatus): void {
    this.db.transaction(() => {
      ...
      if (oldStatus !== status) {
        ...
        this.db.prepare(
          'UPDATE tasks SET status = ?, closed_at = ?, sort_order = ? WHERE id = ?'
  ...
  reorder(projectPath: string, taskNumber: number, newStatus: TaskStatus, targetIndex: number): void {
    this.db.transaction(() => {
      ...
      const oldStatus = task.status;
      const closedAt = newStatus === 'done'
        ? new Date().toISOString()
        : (oldStatus === 'done' ? null : task.closed_at);

      // Get tasks in the target column (excluding the moved task), ordered
      const columnTasks = this.db.prepare(
      ...
      this.db.prepare(
        'UPDATE tasks SET status = ?, closed_at = ? WHERE id = ?'
      ).run(newStatus, closedAt, task.id);
  ...
  delete(projectPath: string, taskNumber: number): void {
    this.db.prepare(
      'DELETE FROM tasks WHERE project_path = ? AND task_number = ?'
