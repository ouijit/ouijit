 ...
 *   - a sandbox-view worktree (~/Ouijit/sandbox-views/<proj>/T-N-sandbox
 *     on child branch s/<user-branch>), which `git worktree add`
 *     populates with tracked files only. No gitignored content ever
 *     lives here, so it's safe to mount into the Lima guest.
 *
 * The `-sandbox` basename keeps the per-worktree git metadata dir at a
 * stable, predictable `<project>/.git/worktrees/T-N-sandbox/` — without
 * it, git would auto-disambiguate against the user worktree's `T-N/`
 * entry and the mount path would be unpredictable.
 *
...
export function getSandboxViewPath(projectName: string, taskNumber: number): string {
  return path.join(getSandboxViewBaseDir(projectName), `T-${taskNumber}-sandbox`);
}
