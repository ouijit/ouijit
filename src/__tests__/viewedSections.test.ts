...
describe('marking a file read a part at a time', () => {
  ...
  test('a file is claimed once every part of it has been, and let go when one is unread', () => {
    const first = markSection(unread, false, parts, '0:a.ts', true);
    ...
    const second = markSection(first.sections, false, parts, '2:a.ts', true);
    ...
    const undone = markSection(second.sections, true, parts, '0:a.ts', false);
  ...
  test('without a lens a file is its own only part', () => {
    expect(markSection(unread, false, ['a.ts'], 'a.ts', true)).toEqual({ sections: new Set(), file: true });
    expect(markSection(unread, true, ['a.ts'], 'a.ts', false)).toEqual({
      sections: new Set(),
      file: false,
    });
  });
  ...
  test('marks against other files are left alone', () => {
    const done = markSection(new Set(['1:b.ts']), false, parts, '0:a.ts', true);
