...
function renderMenu(notes = NOTES) {
  const onJump = vi.fn();
...
describe('handing diff notes to the agent', () => {
  ...
  test('copies the notes as text an agent can act on', async () => {
    renderMenu();
    fireEvent.click(screen.getByLabelText('Copy for the agent'));
    ...
    // The second note is on a line that is gone, and says so.
    expect(written[0]).toContain('src/b.ts:40 (removed line)');
  });

  test('pastes into the terminal as a paste, not as typing', () => {
    renderMenu();
    fireEvent.click(screen.getByLabelText('Paste into the terminal'));
