...
describe('buildWorktreeStartEnv', () => {
  ...
  test('sources the description from the live task, not the stale worktree snapshot', () => {
    const env = buildWorktreeStartEnv({
      hookType: 'continue',
      projectPath: PROJECT,
      // Stale snapshot from worktree-creation time.
    ...
    expect(env.OUIJIT_TASK_DESCRIPTION).toBe('edited description');
  });

  test('keeps OUIJIT_TASK_PROMPT as a deprecated alias of OUIJIT_TASK_DESCRIPTION', () => {
    const env = buildWorktreeStartEnv({
      hookType: 'continue',
      projectPath: PROJECT,
      worktreeInfo: makeWorktree(),
    ...
    expect(env.OUIJIT_TASK_PROMPT).toBe(env.OUIJIT_TASK_DESCRIPTION);
  });

  test('omits the description vars entirely when the live task has no description', () => {
    const env = buildWorktreeStartEnv({
      hookType: 'continue',
      projectPath: PROJECT,
      worktreeInfo: makeWorktree(),
