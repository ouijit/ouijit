...
test('terminal reconnect after reload does not produce % artifacts', async ({ appPage, testRepo }) => {
  ...
  // automatically and its terminal renders there; wait for the reconnected
  // xterm container (view-agnostic) before reading its buffer.
  await expect(appPage.locator('.terminal-xterm-container').first()).toBeAttached({ timeout: 15_000 });
  await expect(appPage.locator('.terminal-xterm-container .xterm-rows').first()).toBeAttached({ timeout: 5_000 });
